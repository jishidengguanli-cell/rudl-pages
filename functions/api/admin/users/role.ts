// functions/api/admin/users/role.ts
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  const role = (body.role || "").trim(); // "admin" | "user"
  if (!uid || !/(^[-\w@.]+$)/.test(uid)) return J({ error: "bad uid" }, 400);
  if (!["admin", "user"].includes(role)) return J({ error: "bad role" }, 400);
