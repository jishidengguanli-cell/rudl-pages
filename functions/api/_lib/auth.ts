...
async function derivePbkdf2(password: string, salt: Uint8Array, iterations: number, keyLen: number): Promise<Uint8Array> {
  ...
  return await pbkdf2Fallback(pwdBytes, salt, iterations, keyLen);
}
...
async function pbkdf2Fallback(password: Uint8Array, salt: Uint8Array, iterations: number, dkLen: number): Promise<Uint8Array> {
  const key = await crypto.subtle.importKey(
    "raw",
    password,
    { name: "HMAC", hash: "SHA-256" },
  ...
  for (let blockIndex = 1; blockIndex <= blockCount; blockIndex++) {
    ...
    let ui = new Uint8Array(await crypto.subtle.sign("HMAC", key, block));
    const t = new Uint8Array(ui);

    for (let i = 1; i < iterations; i++) {
      ui = new Uint8Array(await crypto.subtle.sign("HMAC", key, ui));
      for (let j = 0; j < t.length; j++) t[j] ^= ui[j];
    }
